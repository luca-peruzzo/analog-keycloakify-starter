...
export class AddRemoveButtonsMultiValuedAttributeComponent {
  ...
  hasRemove = computed(() => {
    ...
    if (attribute && values && fieldIndex) {
      return this.getButtonToDisplayForMultivaluedAttributeField({ attribute, values, fieldIndex }).hasRemove;
    }
    return false;
  });
  hasAdd = computed(() => {
    ...
    const fieldIndex = this.fieldIndex();
    if (attribute && values && fieldIndex) {
      return this.getButtonToDisplayForMultivaluedAttributeField({ attribute, values, fieldIndex }).hasAdd;
    }
    return false;
  });
